...
def read_pod_manifest(ctx: click.Context, param: click.Option | click.Parameter, value: str) -> dict[str, t.Any]:
...
@click.option("--log-level", envvar="LOG_LEVEL", default="INFO", help="log level", show_default=True, show_envvar=True)
@click.option(
    "--graylog-host",
    envvar="GRAYLOG_HOST",
    default=None,
...
@click.option(
    "--graylog-port", envvar="GRAYLOG_PORT", default=12201, help="Graylog port", show_default=True, show_envvar=True
)
@click.option(
    "--k8s-in-cluster",
...
def run_with_options(**options: t.Any) -> None:
    ...
    k8s_config = K8sConfig(in_cluster=options["k8s_in_cluster"], namespace=options["k8s_namespace"])
    ...
    graylog_config: GraylogParameters | None = None
